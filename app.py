...
def load_data_from_github(filename: str):
    ...
    try:
        ...
        if e.response.status_code == 404:
            return (
                pd.DataFrame(),
                f"File not found: `{filename}`. It may not have been created yet.",
            )
...
def load_all_weather_data():
    ...
    with st.spinner("Fetching latest weather data from GitHub..."):
        ...
        if errors:
            st.warning(
                "Some data could not be loaded. The data files may be generating.",
                icon="⚠️",
            )
        ...
        if not all_data:
            return pd.DataFrame(
                columns=["city", "timestamp", "temperature", "humidity"]
            )
...
def create_time_series_chart(df: pd.DataFrame, y_value: str, title: str, y_title: str):
    ...
    if df.empty:
        return go.Figure().update_layout(
            title=title,
            annotations=[
                dict(text="No data available for this selection.", showarrow=False)
            ],
        )
...
def create_comparison_bar_chart(df: pd.DataFrame, value_col: str, title: str):
    ...
    if df.empty:
        return go.Figure().update_layout(
            title=title, annotations=[dict(text="No data available.", showarrow=False)]
        )
...
def display_summary_statistics(df: pd.DataFrame, metric: str):
    ...
    for index, row in summary.iterrows():
        with cols[index % 3]:
            st.metric(label=f"📍 {row['city']} Average", value=f"{row['mean']:.1f}")
            st.metric(label=f"📈 {row['city']} Max", value=f"{row['max']:.1f}")
            st.metric(label=f"📉 {row['city']} Min", value=f"{row['min']:.1f}")
            st.markdown("---")
...
def main():
    ...
    with cache_col1:
        if st.button("🔄 Refresh Data", help="Clear data cache and fetch fresh data"):
            st.cache_data.clear()
    ...
    with cache_col2:
        if st.button("🧹 Clear All Cache", help="Clear all dashboard caches"):
            clear_all_caches()
            st.rerun()
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Overview & Comparison", "🌡️ Temperature Analysis", "💧 Humidity Analysis"]
    )
    ...
    with tab1:
        ...
        if filtered_df.empty:
            ...
            with col1:
                temp_bar_chart = create_comparison_bar_chart(
                    filtered_df, "temperature", "Latest Temperature Comparison (°C)"
                )
                st.plotly_chart(temp_bar_chart, use_container_width=True)
            with col2:
                humidity_bar_chart = create_comparison_bar_chart(
                    filtered_df, "humidity", "Latest Humidity Comparison (%)"
                )
